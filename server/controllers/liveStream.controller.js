...
import { exportTicketCode } from "../scripts/exports_data/export_ticket";
import path from "path";
import {getStreamDocuments} from "../services/document.services";
import logger from '../util/log';
import { getRoom } from '../components/room/room.service';
...
export async function addLiveStream(req, res) {
  try {
    ...
    if(data.videoId){
      ...
      try {
        liveStream.documents = await getStreamDocuments(liveStream._id.toString());
      } catch ( error ) {
        console.error('addLiveStream getStreamDocuments error');
        console.error(error);
      }
      ...
      let userId = [];
      let invitedUser = {};
      sendNotificationWhenLiving(array_membership, liveStream.course ? await Course.findById(liveStream.course):liveStreamInfo, liveStream.url, liveStream.course ? 'course':'webinar').catch((error) => {
        logger.error(`Live stream controller, sendNotificationWhenLiving error: ${error.toString()}`);
      });
      ...
      if (liveStream.privacy.to === 'ticket'){
        ...
        liveStream_Services.sendNotificationTicket(options).catch((error) => {
          logger.error(`Live stream controller, sendNotificationTicket error: ${error.toString()}`);
        });
      }
...
export async function getStream(req, res) {
  try {
    ...
    // }
    if(stream.background && JSON.stringify(stream.background) !== '{}'){
      stream.background = `cloud/files/lesson-video/${stream.background._id.toString()}/${stream.background.fileName}`;
    }
...
export async function resetUnCommitStream() {
  try {
    const unCommitStreams = await LiveStream.find({status: 'living'});
    logger.logX('unCommitStreams');
    logger.logX(unCommitStreams);
    return await Promise.all(unCommitStreams.map(async (stream) => {
      const streamId = stream._id.toString();
      // Check room status
      logger.info('unCommitStream', streamId);
      const room = await getRoom({
        meetingID: streamId,
      });
      logger.logX('unCommitStream, room:');
      logger.logX(room);
      if (!room?.meetingID) {
        await liveStream_Services.changeStreamStatus(streamId, 'stopped');
      }
      return true;
    }));
  } catch (error) {
...
export async function getStreamHome(req,res) {
  try{
    ...
    if(userName){
      let userStream = await User.findOne({userName:userName}).lean();
      if(userStream){
        options.user_stream = userStream._id
      }
    }
    //Stream Living
...
export async function exportTicket(req, res) {
  try {
    let webinarId = req.query.id;
    if(!StringHelper.isObjectId(webinarId)) {
      return res.status(404).json({success: false, error: 'Webinar not found.'});
    }
    let userId = req.user._id || null;
    let title = await exportTicketCode(webinarId, userId);
...
export async function getStreamCourseForAdmin( req, res ) {
  ...
  // Update stream info
  try {
    const streams = await LiveStream.find({course: courseId}, 'title status').lean();
    await Promise.all(
      streams.map(async (stream) => {
        stream.streamTimes = await LiveStreamTrackingService.getStreamingTimeForStream(stream._id);
        return stream;
      })
    );
    if ( streams ) {
