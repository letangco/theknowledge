...
Q.process(globalConstants.jobName.REMIND_TICKET,1,async function (job,done){
  try{
    let db = job.data;
    ...
    let students = data.booked;
    students.map(async e => {
      if(e.toString() !== data.user.toString()){
...
Q.process(globalConstants.jobName.REMIND_DAY_TICKET,1,async function (job,done){
  try{
    let db = job.data;
    let data = await LiveStream.findById(db.scheduleId).lean();
    let user = await User.findById(data.user).lean();
